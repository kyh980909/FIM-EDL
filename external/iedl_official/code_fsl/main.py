...
def main(config_dict):
    ...
    task_start = config_dict.get('task_start', 0)
    ...
    info_beta = config_dict.get('info_beta', 1.0)
    info_gamma = config_dict.get('info_gamma', 1.0)
    optimizer_name = config_dict.get('optimizer_name', 'lbfgs')
    lbfgs_lr = config_dict.get('lbfgs_lr', 1.0)
    lbfgs_line_search_fn = config_dict.get('lbfgs_line_search_fn', None)
    adam_lr = config_dict.get('adam_lr', 1e-2)
    ...
    for setting in itertools.product(n_ways_list, n_shots_list, split_name_list, fisher_coeff_list):
        ...
        ndatas = task_maker.GenerateRunSet(start=task_start, end=task_start + n_tasks, cfg=cfg)
        ...
        ood_ndatas = ood_task_maker.GenerateRunSet(start=task_start, end=task_start + n_tasks, cfg=ood_cfg)
        ...
        for ii, run_idxs in enumerate(all_run_idxs):
            ...
            if model_type == 'evnet':
                classifier = train_iedl(support_data, support_label, loss_type, act_type, fisher_coeff,
                                        info_beta=info_beta, info_gamma=info_gamma,
                                        max_iter=lbfgs_iters, verbose=False, use_wandb=use_wandb, n_ep=ii,
                                        optimizer_name=optimizer_name,
                                        lbfgs_lr=lbfgs_lr, lbfgs_line_search_fn=lbfgs_line_search_fn,
                                        adam_lr=adam_lr, grad_clip_norm=grad_clip_norm)
            else:
        ...
        if store_results:
            ...
            for task_id in range(n_tasks):
                ...
                row_dict['task_id'] = task_start + task_id
                for key, value in metrics.items():
