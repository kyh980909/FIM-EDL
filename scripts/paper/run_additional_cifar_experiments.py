...
def _common_cifar_cfg(experiment_name: str, seed: int, suite: str):
    cfg = _load_cfg(experiment_name)
    cfg.seed = seed
    cfg.experiment.suite = suite
    cfg.model.backbone = "vgg16"
    cfg.data.id = "cifar10"
    cfg.data.ood_list = ["svhn", "cifar100"]
...
def _run_controller(seed: int, suite: str) -> None:
    variants = [
        ("cifar_fim_detach", "fisher", "exp", True, 1.0, 1.0),
        ("cifar_fim_nodetach", "fisher", "exp", False, 1.0, 1.0),
        ("cifar_alpha0_gate", "alpha0", "exp", True, 1.0, 1.0),
        ("cifar_constant_gate", "fisher", "constant", True, 1.0, 1.0),
    ]
    for method_variant, info_type, gate_type, detach_weight, beta, gamma in variants:
        cfg = _common_cifar_cfg("info_edl", seed=seed, suite=suite)
...
def _run_fixed(seed: int, suite: str, lambda_value: float, variant: str) -> None:
    cfg = _common_cifar_cfg("edl_l1", seed=seed, suite=suite)
...
def _run_sensitivity(seed: int, suite: str) -> None:
    for beta in (0.5, 1.0, 2.0):
        for gamma in (0.5, 1.0, 2.0):
            cfg = _common_cifar_cfg("info_edl", seed=seed, suite=suite)
            cfg.experiment.method_variant = f"cifar_bg_b{beta}_g{gamma}"
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    parser.add_argument("--suite", default="cifar_additional_20260319")
    ...
    for seed in args.seeds:
        if args.run_fixed:
            _run_fixed(seed=seed, suite=args.suite, lambda_value=1.0, variant="cifar_fixed_l1_matched")
